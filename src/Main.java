...
public class Main {
	...
	public static void main(String[] args) {
		...
		// QueryMessage msg=new QueryMessage("Lord of the rings", "127.0.0.1",
		// 10001, 4);
		// Sender sender=new Sender();
		// sender.sendUDP(msg.toString(), msg.ip_to, msg.port_to);
		// Adding files to the node
		int numOfFiles = 4;
		String[] fileNames = { "Adventures of Tintin", "Jack and Jill", "Glee",
				"Harry Potter" };
		...
		for (int i = 0; i < sharer.neighbors.size(); i++) {
			QueryMessage msg = new QueryMessage("Adventures of Tintin",
					sharer.neighbors.get(i).ip, sharer.neighbors.get(i).port, 0);
